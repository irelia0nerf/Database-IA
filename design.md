...
## 12. Partner Brand Integration

### NVIDIA

**Signal color (NVIDIA contexts only):**

```css
--color-nvidia-signal: #B8D400;   /* lime green — borders, labels, bullet points */
--color-nvidia-dim:    #1A2200;   /* dark tint — container background behind NIM elements */
```

**Usage rules:**

- `--color-nvidia-signal` is the border color of the NIM Container block
- Apply to: NIM container border, “POWERED BY NVIDIA NIMS” badge, “DATA NEVER LEAVES THE PERIMETER” label, bullet points in NVIDIA-sourced solution sections
- The NVIDIA wordmark (`NVIDIA` logotype in white on dark) renders inside the NIM container at its native aspect ratio — do not recolor
- “POWERED BY NVIDIA NIMS” badge: pill shape, `background: transparent`, `border: 1px solid --color-nvidia-signal`, text in `--color-nvidia-signal`, `--font-ui` 9px 500 uppercase, `letter-spacing: 0.08em`
- CMEK SECURED lock icon: `--color-nvidia-signal`, 24px, centered below NIM container
- “NIM CONTAINER” label: `--font-ui` 11px 500 uppercase, `--color-text-primary`; “FoundLab Guardian AI” sublabel: `--font-ui` 10px 400, `--color-text-secondary`
- NVIDIA branding never appears in operational/command center screens — only in product architecture diagrams and landing sections describing the inference layer
- Do not use `--color-nvidia-signal` outside NVIDIA-attributed elements — it conflicts with FoundLab’s own signal color and creates semantic ambiguity

**Container pattern (NIM block):**

```
background: --color-nvidia-dim
border: 1px solid --color-nvidia-signal
border-radius: --radius-md (6px)
padding: 24px
```

Dashed connector lines (vertical, representing inference flow) use `--color-nvidia-signal` at 40% opacity, `stroke-dasharray: 4 4`.

-----

### Google Cloud

**Signal color (GCP contexts only):**

```css
--color-gcp-blue:   #4285F4;   /* Google Blue — badges, partner attribution */
--color-gcp-dim:    #0D1A3A;   /* dark tint — GCP context backgrounds */
```

**Usage rules:**

- Google Cloud logo renders at native full-color — never recolored, never monochrome
- “Powered by Google Cloud” attribution: `--font-ui` 11px 400, `--color-text-secondary`; Google Cloud logo inline at 16px height
- GCP partnership badges (SELECT Technology Partner): `border: 1px solid --color-gcp-blue`, background `--color-gcp-dim`, text `--color-text-primary`
- GCP stack references in architecture diagrams (Cloud Run, Cloud KMS, BigQuery, Vertex AI, Cloud Spanner) use `--color-text-secondary` labels in `--font-mono` 10px — no GCP blue on individual service labels, only on the partner attribution line
- Google Cloud branding appears in: footer attribution, product architecture sections, GCP Marketplace landing context
- Never place Google Cloud branding adjacent to NVIDIA branding without a visual separator — two partner brand signals in the same viewport zone dilute both

**Footer attribution pattern:**

```
[Google Cloud logo 16px] · Google Cloud SELECT Technology Partner
font: --font-ui 11px 400, color: --color-text-tertiary
```

-----

### Partner Brand Coexistence Rules

- NVIDIA and Google Cloud may coexist in architecture diagrams — separate containers, never merged
- Neither partner color (`--color-nvidia-signal`, `--color-gcp-blue`) bleeds into FoundLab UI chrome
- FoundLab’s own `--color-signal` (`#C2D1FF`) remains reserved for cryptographic verification — it must never be confused with either partner signal color
- All partner attribution copy: “Powered by” or “Built on” — never “Certified by” unless certification is formally current and verifiable
